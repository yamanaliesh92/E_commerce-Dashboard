...
type OrderFormValue = z.infer<typeof formSchema>;

export default function BillboardFrom({ initialData }: OrderFromProps) {
  ...
  const onSubmit = async (data: OrderFormValue) => {
    try {
      ...
      if (initialData) {
        await axios.patch(
          `http://localhost:3000/api/store/${params.storeId}/orders`,
          data
        );
      } else {
        await axios.post(
